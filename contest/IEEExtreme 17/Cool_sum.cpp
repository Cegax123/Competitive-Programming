...
struct ModInt
{
    ...
    M operator*(const M &r) const { return M().set_v(ull(v) * r.v % MOD); }
...
int main()
{
    ...
    while(n){
        if(n&1){
            auto b=multiply(ans,a);
            for(int i=0;i<(1<<k);i++) ans[i]=0;
            for(int i=0;i<b.size();i++) ans[i%(1<<k)]+=b[i];
        }
        n>>=1;
        auto b=multiply(a,a);
        for(int i=0;i<(1<<k);i++) a[i]=0;
        for(int i=0;i<b.size();i++) a[i%(1<<k)]+=b[i];
    }
    for(int i=0;i<(1<<k);i++) cout<<ans[i].v<<" ";
    return 0;
}
