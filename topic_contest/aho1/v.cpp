...
Mat mul(const Mat& A, const Mat& B) {
	...
	Mat res(a, vector<ll>(c, -1));

	for(int i = 0; i < a; i++) {
		for(int j = 0; j < b; j++) {
			for(int k = 0; k < c; k++) {
				if(A[i][j] >= 0 and B[j][k] >= 0) res[i][k] = max(res[i][k], A[i][j] + B[j][k]);
			}
		}
	}
...
int main() {
	...
	Mat A(nodes, vector<ll>(nodes, -1)), B(nodes, vector<ll>(1, -1));
	B[0][0] = 0;
